...
The built-in `run_command` tool executes an approved program directly, without a shell. It accepts `program`, optional `args`, optional `cwd`, and a timeout (default 120 seconds, maximum 300 seconds). Its working directory must also be covered by `--allow-read`. It captures stdout and stderr with 64 KiB limits. Programs require explicit repeatable `--allow-exec PROGRAM` permission. Each command runs in a separate Unix process group or Windows Job Object. Timeout or cancellation terminates that group/job, including descendants, and application shutdown waits for cancelled commands to be reaped. Remaining group/job members are also terminated when output collection finishes normally. Unix processes that deliberately create a new session or leave the group are outside this cleanup boundary; execution is not sandboxed.
